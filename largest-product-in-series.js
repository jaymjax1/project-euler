// The four adjacent digits in the 1000-digit number that have the greatest product are 9 × 9 × 8 × 9 = 5832.

// Find the thirteen adjacent digits in the 1000-digit number that have the greatest product. What is the value of this product?

/** Strategy:
 * Have a left and right pointer than chunks the number into array of 13 elements. Multiple those together and add the key, (which is the sorted values), and the value, (whic is the product), iterate to the next chunk. Get that chunk, sort it, and see if it already appears in the table, if so, continue, if not, do step 2. Do this until the right point equals index of 999
 */

 /**
  * substr (MDN) takes parameters as (from, length).
  * substring (MDN) takes parameters as (from, to). 
  * use the split() to return an array from strings!
  * */

var utils = require("./utils/utils");

const bigNumber = ""

const chunks = {};
const getChunks = (num, size) => {
  for (let i = 0, j = size; j <= num.length; i++, j++) {
      if (!chunks[num.substring(i,j).split("").sort().join("")]) {
        let value = getProduct(num.substring(i,j))
        chunks[num.substring(i,j).split("").sort().join("")] = value
      }
  }
};

const getProduct = (stringNum) => {
    return stringNum.split("").reduce((acc,curr) => {
        return parseInt(curr) * acc
    }, 1)
}

const getMaxCombo = (number, chunk) => {
    getChunks(number, chunk);
    return Math.max(...Object.values(chunks))
}

console.log(getMaxCombo(bigNumber,13));